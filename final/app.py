...
def generate_map():
    try:
        ...
        # Generate crime map
        mumbai_map = folium.Map(location=[user_lat, user_lon], zoom_start=12)
        ...
        for lat, lon, status, officer, crime_type, case_id, year, distance in filtered_cases:
            ...
            # Draw a line from the user to the crime location
            folium.PolyLine([(user_lat, user_lon), (lat, lon)], color="blue", weight=2.5, opacity=0.8).add_to(mumbai_map)

        # Save map inside templates directory
        map_filename = "mumbai_filtered_crime_map.html"
        map_path = os.path.join(TEMPLATE_DIR, map_filename)
        mumbai_map.save(map_path)

        return jsonify({"message": "Crime map generated!", "map_url": "/view_map"})
...
def view_map():
    return render_template("mumbai_filtered_crime_map.html")
